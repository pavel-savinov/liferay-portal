	...
	protected void renameResourceActions() throws UpgradeException {
		...
		try (LoggingTimer loggingTimer = new LoggingTimer();
			PreparedStatement ps = connection.prepareStatement(
				selectSb.toString(), ResultSet.TYPE_SCROLL_INSENSITIVE)) {
				...
				while (rs.next()) {
					...
					if (categoriesActions != 0) {
						...
						if (primKey.equals(AssetPermission.RESOURCE_NAME)) {
							primKey = AssetPermission.RESOURCE_NAME_CATEGORIES;
						}
					...
					if (tagsActions != 0) {
						...
						if (primKey.equals(AssetPermission.RESOURCE_NAME)) {
							primKey = AssetPermission.RESOURCE_NAME_TAGS;
						}

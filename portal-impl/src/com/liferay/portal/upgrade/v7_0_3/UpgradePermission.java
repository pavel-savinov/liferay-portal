...
public class UpgradePermission extends UpgradeProcess {
	...
	protected void renameResourceActions() throws UpgradeException {
		String selectQuery =
			"select count(*) from ResourceAction where name in (?,?)";

		try (LoggingTimer loggingTimer = new LoggingTimer();
			PreparedStatement ps = connection.prepareStatement(
				selectQuery)) {

			ps.setString(1, AssetCategoriesPermission.RESOURCE_NAME);
			ps.setString(2, AssetTagsPermission.RESOURCE_NAME);
			...
			sb.append(AssetCategoriesPermission.RESOURCE_NAME);
			...
			sb.append("update ResourceAction set name = '");
			sb.append(AssetTagsPermission.RESOURCE_NAME);
			sb.append("' where name = '");
			sb.append(AssetPermission.RESOURCE_NAME);
			...
			try (PreparedStatement ps2 = connection.prepareStatement(
				...
				ps2.setString(2, AssetTagsPermission.RESOURCE_NAME);
			...
			try (ResultSet rs = ps.executeQuery();
				...
				while (rs.next()) {
					...
					if (categoriesActions != 0) {
						...
						ps3.setString(
							3, AssetCategoriesPermission.RESOURCE_NAME);
						ps3.setInt(4, scope);

						if (primKey.equals(AssetPermission.RESOURCE_NAME)) {
							ps3.setString(
								5, AssetCategoriesPermission.RESOURCE_NAME);
						}
						else {
					...
					if (tagsActions != 0) {
						ps3.setLong(1, increment());
						ps3.setLong(2, companyId);
						ps3.setString(3, AssetTagsPermission.RESOURCE_NAME);
						ps3.setInt(4, scope);

						if (primKey.equals(AssetPermission.RESOURCE_NAME)) {
							ps3.setString(5, AssetTagsPermission.RESOURCE_NAME);
						}
						else {
							ps3.setString(5, primKey);
						}
